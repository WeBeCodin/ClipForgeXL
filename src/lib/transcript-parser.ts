...
export class TranscriptParser {
  static async parseFile(file: File): Promise<ParsedTranscript> {
    ...
    switch (extension) {
      ...
      default:
        throw new Error(
          `Unsupported transcript format: "${extension}". Supported formats are: SRT, VTT, JSON, TXT, DOC, DOCX, and PDF.`
        );
    }
  }
  ...
  private static async extractTextFromWord(file: File): Promise<string> {
    try {
      ...
      throw new Error(
        `Word document processing requires server-side support. Please save your document as a .txt file or copy the text into a .txt file for now.`
      );
    } catch (error) {
      throw new Error(
  ...
  private static async extractTextFromPDF(file: File): Promise<string> {
    try {
      ...
      throw new Error(
        `PDF processing requires server-side support. Please copy the text from your PDF into a .txt file for now.`
      );
    } catch (error) {
      throw new Error(
  ...
  private static parseVTT(content: string): ParsedTranscript {
    ...
    for (const block of blocks) {
      ...
      const startTime = this.parseTimeVTT(
        timeMatch[1],
        timeMatch[2],
        timeMatch[3]
      );
      const endTime = this.parseTimeVTT(
        timeMatch[4],
        timeMatch[5],
        timeMatch[6]
      );
  ...
  private static parseJSON(
    ...
    try {
      ...
      if (!trimmedContent.startsWith("{") && !trimmedContent.startsWith("[")) {
        throw new Error(
          `File does not appear to be valid JSON. Content starts with: "${trimmedContent.substring(
            0,
            50
          )}..."`
        );
      ...
      if (data.words && Array.isArray(data.words)) {
        ...
        if (data.words.length > 0 && !this.isValidWordFormat(data.words[0])) {
          throw new Error(
            'JSON transcript format is incorrect. Expected words with "word", "start", "end", and "punctuated_word" properties.'
          );
        }
      ...
      if (Array.isArray(data)) {
        ...
        if (data.length > 0 && !this.isValidWordFormat(data[0])) {
          throw new Error(
            'JSON transcript format is incorrect. Expected array of words with "word", "start", "end", and "punctuated_word" properties.'
          );
        }
        return { words: data };
      }
      ...
      throw new Error(
        "Invalid JSON transcript format. Expected either {words: [...]} or [...]"
      );
    } catch (error) {
      if (error instanceof SyntaxError) {
        const fileInfo = fileName ? ` in file "${fileName}"` : "";
        throw new Error(
          `Failed to parse JSON transcript${fileInfo}: ${error.message}`
        );
      }
      throw error;
    }
